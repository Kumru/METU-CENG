...
public:
	...
	Vector3D backgroundColor;		// Background color
	Vector3D ambientLight;			// Ambient light radiance
	...
	vector<Vector3D> vertices;		// Vector holding all vertices (vertex data)
...
private:
	...
	Vector3D TracePixel(Ray& ray);
	void RunTracing(Camera* cam, Image& image, int min_h, int max_h);
};
