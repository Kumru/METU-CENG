...
# I traced a similar way with the SEQ language to add the IADDQ
# instruction. I will add a table like the ones in the book.
#
# Also, I will add a comment as CHANGED to the lines I have changed.
#
# Fetch:
#	icode:ifun<-M1[PC]
#       rA:rB<-M1[PC+1]
...
bool need_valC =
	f_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IJXX, ICALL, IIADDQ }; # CHANGED
...
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, IIADDQ } : D_rB; # CHANGED
...
word d_dstE = [
	D_icode in { IRRMOVQ, IIRMOVQ, IOPQ, IIADDQ } : D_rB; # CHANGED
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
...
word aluA = [
	...
	E_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ } : E_valC; # CHANGED
...
word aluB = [
	E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL, 
		     IPUSHQ, IRET, IPOPQ, IIADDQ } : E_valB; # CHANGED
...
bool set_cc = ( E_icode == IOPQ || E_icode == IIADDQ ) && # CHANGED
